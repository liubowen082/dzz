        ...
        getHTML(data, function (obj) {
            ...
            if(!userdata){
                ...
                if (obj.html.approver_list.length != 0) {
                    ...
                    $('#manger_audit').parent().parent().remove();
                }
    ...
    function addEvt() {
        ...
        window.check_handle = function (ele) {
            ...
            senRequestHandle('/index.php?mod=shenpi&op=index&act=application_check', args, function () {
                target.attr('loading', 'false');
                $('[node_id="' + args.id + '"]', mainBox).find('.office_status_8').replaceWith(tpl.checked('已核销'));
                ui.box.close();
            }, function () {
                ui.box.close();
                target.attr('loading', 'false');
        ...
        //核销功能
        mainBox.delegate('a[node_type="check"]', 'click', function (e) {
            var target = $(e.target), args = target.attr('node_args');
