...
define(function(require, exports, module) {
	...
	var moduleList = {
		...
		'data_list' : { // 清单
			'name':'清单',
			'tpl' : '<dl rel="data_list" id="#{id}"><dt>#{isMust} #{title}：</dt><dd><div>#{value}<div class="js_total">总计：<span node-name="data-list-all">0</span>元</div></div></dd></dl>',
			'value' : '<div class="data-list js_list"><input type="text" class="q-txt" placeholder="名称" style="width:150px" value="#{value1}"><input type="text" class="q-txt" placeholder="#{option}" style="width:60px"  value="#{value2}" node-name="data-list-num"><span class="vline">#{option1}</span><div class="widget-date" #{data_format_display}><input type="text" class="q-txt rcalendar_input" onclick="laydate()" readonly="readonly" placeholder=""  value="#{value3}"><i class="icon-clock2"></i></div><span class="icon-plus js_add" node-name="data_list_add"  option="#{option}" option_else="#{option_else}" data_option="#{data_option}" data_format="#{data_format}"></span></div>',
			'valueOther' : '<div class="data-list js_list"><input type="text" class="q-txt" placeholder="名称" style="width:150px" value="#{value1}"><input type="text" class="q-txt" placeholder="#{option}" style="width:60px" value="#{value2}" node-name="data-list-num"><span class="vline">#{option1}</span><div class="widget-date" model-node="date_widget_load" #{data_format_display}><input type="text" class="q-txt rcalendar_input" onclick="laydate()" placeholder="" value="#{value3}"><i class="icon-clock2"></i></div><span class="js_add icon-close" node-name="data_list_remove" data_format="#{data_format}"></span></div>'
		},
		'user' : {
			'name':'人名输入框',
		...
		'attach' : {
			'name':'附件',
			'tpl' : '<dl rel="attach" id="#{id}"><dt>#{title}：</dt><dd><div model-node="upload_widget"><div class="qq-uploader"><div class="qq-upload-drop-area" style="display:none"><span>拖动文件到此完成上传</span></div><div class="qq-upload-button" style="position: relative; overflow: hidden; direction: ltr;"><div class="qq-upload-text"><i class="icon-paperclip"></i><span>添加文件</span></div><div class="qq-drop-drag">或拖拽到此上传</div><input multiple="multiple" type="file" name="file" style="position: absolute; right: 0px; top: 0px; font-family: Arial; font-size: 118px; margin: 0px; padding: 0px; cursor: pointer; opacity: 0;"></div><span class="qq-drop-processing"><span>请稍后再拖动文件...</span><span class="qq-drop-processing-spinner"></span></span></div></div><div class="qq-uploader"><ul class="qq-upload-list new-upload-list"  node-name="upload_list">#{valueList}</ul></div><input type="hidden" value="#{value}" node-name="input"></dd></dl>',
			'value' : '<li class="qq-upload-success" id="#{listId}"><span class="qq-upload-finished"></span><a href="#{url}" event-node="feed_ajax_detail" title="#{title}" target="_blank"><span class="qq-upload-icon"><i class="qg-ico16-file ico-#{type}"></i></span></a><a class="qq-upload-delete icon-close" node-name="fileDel" li-id="#{listId}"  href="javascript:;" li-file="#{title}|#{url}"></a><span class="qq-upload-size">#{size}</span><span class="qq-upload-file"><a href="#{url}" event-node="feed_ajax_detail" title="#{title}" target="_blank">#{shortTitle}</a></span><span class="qq-upload-download"><a href="#{downUrl}" class="icon-download" target="_blank"></a></span><span class="qq-upload-status-text"></span></li>'
		},
		'number' : {
			'name':'数字',
